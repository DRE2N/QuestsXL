...
public class QDialogueStage {
    ...
    public static QDialogueStage loadFromConfig(String id, ConfigurationSection section) {
        ...
        if (conditionSection != null) {
            try {
                conditions.addAll(ConditionManager.loadConditions(id + ", conditions", conditionSection));
            } catch (Exception e) {
                QuestsXL.getInstance().getErrors().add(new FriendlyError(id, "Conditions konnten nicht geladen werden.", e.getMessage(), "Wahrscheinlich falsche Einrückung."));
            }
        }
        List<QAction> postActions = new ArrayList<>();
        ConfigurationSection rewardActionSection = section.getConfigurationSection("onFinish");
        if (rewardActionSection != null) {
            try {
                postActions.addAll(ActionManager.loadActions(id + ", onFinish", rewardActionSection));
            } catch (Exception e) {
