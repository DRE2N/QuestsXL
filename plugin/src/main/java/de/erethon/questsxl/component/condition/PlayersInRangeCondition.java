...
public class PlayersInRangeCondition extends QBaseCondition implements VariableProvider {
    ...
    public boolean checkInternal(Quester quester) {
        ...
        if (mode == Mode.PARTICIPATING || mode == Mode.IN_RANGE_EVENT) {
            return checkInRange(event, quester);
        }
        if (mode == Mode.IN_RANGE_PLAYER && quester instanceof QPlayer player) {
            lastPlayerCount = player.getPlayer().getLocation().getNearbyPlayers(range).size();
            if (lastPlayerCount >= minPlayers && lastPlayerCount <= maxPlayers) {
    ...
    private boolean checkInRange(QEvent e, Quester quester) {
        QEvent conditionEvent = event != null ? event : e;
        if (conditionEvent == null) {
            return success(quester);
        }
        if (mode == Mode.IN_RANGE_EVENT) {
            lastPlayerCount = conditionEvent.getPlayersInRange().size();
        } else if (mode == Mode.PARTICIPATING) {
            lastPlayerCount = conditionEvent.getParticipants().size();
        }
        if (lastPlayerCount >= minPlayers && lastPlayerCount <= maxPlayers) {
            return success(quester);
        }
    ...
    /** Exposes %players_in_range% to child actions (onSuccess / onFail). */
    @Override
    public Map<String, QVariable> provideVariables(Quester quester) {
        return Map.of("players_in_range", new QVariable(lastPlayerCount));
    }
