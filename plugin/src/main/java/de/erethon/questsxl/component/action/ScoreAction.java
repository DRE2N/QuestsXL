...
public class ScoreAction extends QBaseAction {
    ...
    @QParamDoc(name = "value", description = "The value to add, remove, set or reset — supports %variables%", def = "1")
    ...
    public void playInternal(Quester quester) {
        ...
        switch (scope) {
            ...
            case PLAYER -> {
                final int resolvedAmount = amount;
                execute(quester, player -> modifyScore(player, resolvedAmount));
            }
        }
        onFinish(quester);
    }

    private void modifyScore(QPlayer player, int amount) {
        switch (operation) {
    ...
    public void load(QConfig cfg) {
        ...
        rawAmount = cfg.getString("value", "1");
