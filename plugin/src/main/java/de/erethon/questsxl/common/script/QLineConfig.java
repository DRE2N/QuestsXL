...
public class QLineConfig implements QConfig {
    ...
    public String getString(String key) {
        return resolve(result.get(key));
    }

    public String getString(String key, String def) {
        return resolve(result.getOrDefault(key, def));
    }
    ...
    public int getInt(String key) {
        return resolveInt(result.get(key));
    }

    public int getInt(String key, int def) {
        return resolveInt(result.getOrDefault(key, String.valueOf(def)));
    }

    public double getDouble(String key) {
        return resolveDouble(result.get(key));
    }

    public double getDouble(String key, double def) {
        return resolveDouble(result.getOrDefault(key, String.valueOf(def)));
    }

    public long getLong(String key) {
        return resolveLong(result.get(key));
    }

    public long getLong(String key, long def) {
        return resolveLong(result.getOrDefault(key, String.valueOf(def)));
    }

    public boolean getBoolean(String key) {
        return resolveBoolean(result.get(key));
    }

    public boolean getBoolean(String key, boolean def) {
        return resolveBoolean(result.getOrDefault(key, String.valueOf(def)));
    }
    ...
    private int resolveInt(String raw) {
        ...
        ExecutionContext ctx = ExecutionContext.current();
        if (ctx != null && raw.contains("%")) {
            return ctx.resolveInt(raw);
        }
        return Integer.parseInt(raw);
    }

    private double resolveDouble(String raw) {
        if (raw == null) throw new NumberFormatException("null");
        ExecutionContext ctx = ExecutionContext.current();
        if (ctx != null && raw.contains("%")) {
            return ctx.resolveDouble(raw);
        }
    ...
    private long resolveLong(String raw) {
        ...
        ExecutionContext ctx = ExecutionContext.current();
        if (ctx != null && raw.contains("%")) {
            return ctx.resolveLong(raw);
        }
        return Long.parseLong(raw);
    }

    private boolean resolveBoolean(String raw) {
        if (raw == null) return false;
        ExecutionContext ctx = ExecutionContext.current();
        if (ctx != null && raw.contains("%")) {
            return ctx.resolveBoolean(raw);
        }
